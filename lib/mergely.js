...
jQuery.extend(Mgly.CodeMirrorDiffView.prototype, {
	init: function(el, options) {
		...
		if (this.settings.fgcolor === 'auto') {
			this.settings.fgcolor = { a: 'auto', c: 'auto', d: 'auto' };
		}
		if ([this.settings.fgcolor.a, this.settings.fgcolor.c, this.settings.fgcolor.d].indexOf('auto') !== -1) {
			// Detect colors from the current border styles
			var testElem = $('<div></div>').attr('style', 'display: none !important').addClass('mergely lhs rhs start').appendTo(this.element);
			for (var attr in this.settings.fgcolor) {
				if(this.settings.fgcolor[attr] !== 'auto') continue;
				testElem.addClass(attr);
				this.settings.fgcolor[attr] = testElem.css('borderTopColor');
				testElem.removeClass(attr);
			}
			testElem.remove();
		}
	...
	_draw_diff: function(editor_name1, editor_name2, changes, additionalHighlights) {
		...
		for (var i = 0; additionalHighlights && i < additionalHighlights.length; ++i) {
			...
			// margin indicators
			var y_start = ((highlight['y-start'] + ex[highlight.side + '_scroller'].scrollTop()) * ex.visible_page_ratio);
			var y_end = ((highlight['y-end'] + ex[highlight.side + '_scroller'].scrollTop()) * ex.visible_page_ratio) + 1;
