...
jQuery.extend(Mgly.CodeMirrorDiffView.prototype, {
	init: function(el, options) {
		...
		var fetchableColors = {
			fgcolor: $.extend({}, this.settings.fgcolor),
		...
		$.extend(this.settings, fetchableColors);
	...
	_replace_auto_colors: function(colors, existingTestElem) {
		var testElem = existingTestElem || $('<div></div>').attr('style', 'display: none !important').appendTo(this.element);
		for(var c in colors) {
	...
	_draw_info: function(editor_name1, editor_name2) {
		...
		var gutter_height = Math.max( // Scrolled content may be different sizes
			jQuery(this.editor[editor_name1].getScrollerElement()).children(':first-child').height(),
			jQuery(this.editor[editor_name2].getScrollerElement()).children(':first-child').height()
		);
