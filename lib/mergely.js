...
jQuery.extend(Mgly.CodeMirrorDiffView.prototype, {
	...
	_scrolling: function(editor_name) {
		...
		for (var name in this.editor) {
			...
			this._layout_changing(editor_name1, editor_name2);
	...
	_layout_changed: function(editor_name1, editor_name2) {
	...
	_calculate_offsets: function (editor_name1, editor_name2, changes, additionalHighlights) {
		...
		var leftScrollTop = jQuery(this.editor[editor_name1].getScrollerElement()).scrollTop() - 4 - lhschc.top; // 4px padding before lines, see .CodeMirror-lines
		var rightScrollTop = jQuery(this.editor[editor_name2].getScrollerElement()).scrollTop() - 4 - rhschc.top;
		...
		for (var i = 0; i < changes.length; ++i) {
			...
			change['lhs-y-start'] = ls.top - leftScrollTop;
			if (change['op'] == 'c' || change['op'] == 'd') {
				change['lhs-y-end'] = le.bottom - leftScrollTop;
			}
			else {
				change['lhs-y-end'] = le.top - leftScrollTop;
			}
			change['rhs-y-start'] = rs.top - rightScrollTop;
			if (change['op'] == 'c' || change['op'] == 'a') {
				change['rhs-y-end'] = re.bottom - rightScrollTop;
			}
			else {
				change['rhs-y-end'] = re.top - rightScrollTop;
			}
		...
		for (var i = 0; i < additionalHighlights.length; ++i) {
			...
			var sideScrollTop = highlight.side === 'lhs' ? leftScrollTop : rightScrollTop;
			...
			highlight['y-start'] = hs.top - sideScrollTop;
			highlight['y-end'] = he.bottom - sideScrollTop;
