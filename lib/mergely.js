...
jQuery.extend(Mgly.CodeMirrorDiffView.prototype, {
	...
	unbind: function() {
		...
		if (this.layout_changed_timeout != null) clearTimeout(this.layout_changed_timeout);
		if (this.resize_handler != null) jQuery(window).unbind('resize', this.resize_handler);
	...
	resize: function() {
		...
		this._layout_changing(this.id + '-lhs', this.id + '-rhs');
	},
	diff: function() {
	...
	bind: function(el) {
		...
		this.layout_changed_timeout = null;
		...
		if (this.settings.autoresize) {
			...
			var sz = function(init) {
				...
				self._layout_changing(self.id + '-lhs', self.id + '-rhs');
			}
	...
	_scrolling: function(editor_name) {
		...
		for (var name in this.editor) {
			...
			this._layout_changed(editor_name1, editor_name2, true); // Happen synchronously to allow illusion of canvases scrolling at same time as editor
	...
	_layout_changing: function(editor_name1, editor_name2) {
		this.trace('layout change', 'layout changing-timeout', this.layout_changed_timeout);
		var self = this;
		if (this.layout_changed_timeout != null) clearTimeout(this.layout_changed_timeout);
		this.layout_changed_timeout = setTimeout(function(){
			var timer = new Mgly.Timer();
			self._layout_changed(editor_name1, editor_name2);
			self.trace('layout change', 'total time', timer.stop());
		}, this.settings.layout_change_timeout);
	},
	_layout_changed: function(editor_name1, editor_name2, offsetsUnchanged) {
		if(!this.changes) return; // 0 length is fine, undefined is not
		var timer = new Mgly.Timer();
		if(!offsetsUnchanged) {
		...
		if(this.settings.viewport) this._markup_changes(editor_name1, editor_name2, this.changes);
	...
	_is_change_in_view: function(vp, change) {
		...
		if ((change['lhs-line-to'] < vp.from || change['lhs-line-from'] > vp.to) &&
			(change['rhs-line-to'] < vp.from || change['rhs-line-from'] > vp.to)) {
			// if the change is completely outside the viewport on both sides, skip
			return false;
		}
		return true;
	},
	_is_highlight_in_view: function(vp, highlight) {
		if (!this.settings.viewport) return true;
		if (highlight['line-to'] < vp.from || highlight['line-from'] > vp.to) {
			// if the highlight is completely outside the viewport, skip
			return false;
		}
		return true;
	},
	_calculate_line_position: function(editor_name, lineNum, firstLineCoords) {
		if(!firstLineCoords) firstLineCoords = this.editor[editor_name].charCoords({line: 0}, 'local'); // Much faster if this is passed in when used repeatedly
		if (this.editor[editor_name].getOption('lineWrapping')) {
			// If using line-wrapping, we must get the height of the line
			var coords = this.editor[editor_name].cursorCoords({line: lineNum, ch: 0}, 'local');
	...
	_calculate_offsets: function (editor_name1, editor_name2, changes, additionalHighlights) {
		...
		var lhschc = this.editor[editor_name1].charCoords({line: 0}, 'local');
		var rhschc = this.editor[editor_name2].charCoords({line: 0}, 'local');
		...
		for (var i = 0; i < changes.length; ++i) {
			...
			change['lhs-y-start'] = ls.top + 4;
			if (change['op'] == 'c' || change['op'] == 'd') {
				change['lhs-y-end'] = le.bottom + 4; // 4px padding in CodeMirror-lines
			}
			else {
				change['lhs-y-end'] = le.top + 4;
			}
			change['rhs-y-start'] = rs.top + 4;
			if (change['op'] == 'c' || change['op'] == 'a') {
				change['rhs-y-end'] = re.bottom + 4;
			}
			else {
				change['rhs-y-end'] = re.top + 4;
			}
		...
		for (var i = 0; i < additionalHighlights.length; ++i) {
			...
			highlight['y-start'] = hs.top + 4;
			highlight['y-end'] = he.bottom + 4;
	...
	_draw_diff: function(editor_name1, editor_name2, changes, additionalHighlights) {
		...
		var leftScrollTop = ex.lhs_scroller.scrollTop();
		var rightScrollTop = ex.rhs_scroller.scrollTop();
		...
		for (var i = 0; i < changes.length; ++i) {
			...
			// margin indicators
			var lhs_y_start = (change['lhs-y-start'] * ex.lhs_visible_page_ratio);
			var lhs_y_end = (change['lhs-y-end'] * ex.lhs_visible_page_ratio) + 1;
			var rhs_y_start = (change['rhs-y-start'] * ex.rhs_visible_page_ratio);
			var rhs_y_end = (change['rhs-y-end'] * ex.rhs_visible_page_ratio) + 1;
			...
			lhs_y_start = change['lhs-y-start'] - leftScrollTop;
			lhs_y_end = change['lhs-y-end'] - leftScrollTop;
			rhs_y_start = change['rhs-y-start'] - rightScrollTop;
			rhs_y_end = change['rhs-y-end'] - rightScrollTop;
		...
		for (var i = 0; additionalHighlights && i < additionalHighlights.length; ++i) {
			...
			// margin indicators
			var y_start = (highlight['y-start'] * visible_page_ratio);
			var y_end = (highlight['y-end'] * visible_page_ratio) + 1;
